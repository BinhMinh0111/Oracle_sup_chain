...
FOR EACH ROW
BEGIN
    IF (TRUNC(SYSDATE) > TRUNC(GET_NGAY_LAP_HD(:OLD.SOHD)))
    THEN RAISE_APPLICATION_ERROR(-20011,'You can not alter this form anymore');
    end if;
END;
...
update CT_hoa_don
set SO_LUONG = 10
where sohd like 'HD00001' AND MASP LIKE 'SP00001';

CREATE OR REPLACE FUNCTION TINH_TONG_TIEN (input_sohd char) RETURN FLOAT
AS
v_TONG_TIEN FLOAT;
BEGIN
...
INTO v_TONG_TIEN
FROM CT_HOA_DON
WHERE SOHD LIKE input_sohd;
RETURN v_TONG_TIEN;
END;

--CREATE OR REPLACE  PROCEDURE NHAP_TONG_TIEN (input_sohd char)
--AS
--v_TONG_TIEN FLOAT;
--BEGIN
--
...
--CREATE OR REPLACE TRIGGER HD_TONG_TIEN
--AFTER INSERT OR UPDATE OF THANH_TIEN
--ON CT_HOA_DON
--FOR EACH ROW
--BEGIN
--UPDATE HOA_DON 
--SET HOA_DON.TONG_TIEN = (TINH_TONG_TIEN(:OLD.SOHD))
--WHERE HOA_DON.SOHD LIKE :OLD.SOHD;
--END;
...
BEGIN
UPDATE HOA_DON 
SET HOA_DON.TONG_TIEN = :NEW.TONG_TIEN
WHERE HOA_DON.SOHD LIKE :NEW.TONG_TIEN;
END;


create OR REPLACE VIEW V_HD_TONG_TIEN
AS
SELECT HOA_DON.SOHD,(SUM(THANH_TIEN)) TONG_TIEN FROM CT_HOA_DON JOIN HOA_DON ON CT_HOA_DON.SOHD = HOA_DON.SOHD GROUP BY HOA_DON.SOHD;
...
AS 
SELECT PN.MA_PHIEU_NHAP, NGAY_LAP_PHIEU, CONCAT(CONCAT(HO_LOT,' '), TENNV) AS HO_TEN ,NHAP_TU,XUAT_DEN, NGAY_NHAP_HANG
FROM PHIEU_NHAP PN JOIN CT_PHIEU_NHAP CTPN ON PN.MA_PHIEU_NHAP = CTPN.MA_PHIEU_NHAP
...
AS 
SELECT PX.MA_PHIEU_XUAT, NGAY_LAP_PHIEU, CONCAT(CONCAT(HO_LOT,' '), TENNV) AS HO_TEN ,NHAP_DEN, XUAT_TU, NGAY_XUAT_HANG
FROM PHIEU_XUAT PX JOIN CT_PHIEU_XUAT CTPX ON PX.MA_PHIEU_XUAT = CTPX.MA_PHIEU_XUAT
...
FOR X IN 1..RETURN_TON_KHO.COUNT
    LOOP
        ...
        begin
            ...
            where (CONCAT(RETURN_TON_KHO(X).THANG,RETURN_TON_KHO(X).NAM))LIKE(CONCAT(TON_KHO.THANG,TON_KHO.NAM)) ;
        if c = 0 then                
        INSERT INTO TON_KHO
...
CREATE OR REPLACE TRIGGER TRIG_UPDATE_TON_KHO
AFTER INSERT OR UPDATE ON CT_THUNG
FOR EACH ROW
BEGIN
PRC_GET_TON_KHO;
END;
